...
class BaseGameAgent(Agent):
    ...
    async def on_enter(self) -> None:
        ...
        # If there was a previous context snapshot, inherit it without holding onto agent instances
        if getattr(userdata, "prev_chat_ctx_items", None):
            items_copy = self._truncate_chat_ctx(
                userdata.prev_chat_ctx_items,
    ...
    def _snapshot_chat_ctx(self) -> list:
        """Return a truncated snapshot of current chat context items for handoff."""
        return self._truncate_chat_ctx(
            self.chat_ctx.items,
            keep_function_call=True,
            keep_last_n_messages=200,
        )
