...
1. **`character.py`** — Character model (local dataclasses for NPC generation)
   - `Character`, `PlayerCharacter`, `NPCCharacter`

2. **`api/client.py`** — HTTP clients for the RPG API
   - `CharacterClient`, `CombatClient`, `InventoryClient`, `TradeClient`, `QuestClient`, `ReferenceClient`

3. **`api/models.py`** — Pydantic response models for API data
   - `APICharacter`, `APICombatSession`, `APIInventoryItem`, `APIQuestAssignment`, etc.

4. **`agent.py`** — Agent entrypoint and RPCs
   - Registers RPC handlers (`get_current_context`, etc.)
   - All RPCs fetch data from the API

5. **`agents/narrator_agent.py`** — Exploration, dialogue, and quest management
   - Function tools for NPC interaction, exploration, skill checks
   - LLM-driven quest management (get, accept, progress, complete quests)

6. **`agents/combat_agent.py`** — Combat turn processing (API-only)

7. **`core/game_state.py`** — `GameUserData` (stores API IDs, not objects)

8. **`core/state_service.py`** — Async API fetch methods for game state

9. **`services/`** — Service layer for API operations
   - `player.py` — Character creation and starting quest assignment
   - `quests.py` — Quest serialization for frontend display
   - `exploration.py` — Zone navigation via API
   - `story.py` — Unified story management

10. **`generators/`** — AI-driven NPC and item generators

11. **`prompts/`**, **`rules/`** — Configuration and prompt templates

> **Note**: All game mechanics (dice rolls, combat resolution, skill checks, spells) are handled by the RPG API (`rpg-backend/`). The agent only stores IDs and fetches state from the API as needed.
...
async def start_combat(...):
    # Create enemies via API and start combat session
    combat = await combat_client.start(player_id=..., enemy_ids=[...])
    state.set_combat_session_id(combat.id)
    # Return CombatAgent to trigger switch
    return CombatAgent(), "Combat begins!"

# When combat ends in CombatAgent
async def _end_combat(self, victory):
    # Resolve rewards and finish combat via API
    await combat_client.resolve(combat_id)
    await combat_client.finish(combat_id)
    state.clear_combat()
    # Return NarratorAgent to go back to exploration
    return NarratorAgent(), "Combat ends!"
```

### UserData Pattern

The `GameUserData` class maintains API IDs and session state across agent switches:
...
class GameUserData:
    ...
    current_npc_ids: List[int] = field(default_factory=list)  # API character IDs
    ...
    # Caches (refreshed from API)
    _player_cache: Optional[APICharacter] = None
    _combat_cache: Optional[APICombatSession] = None
```
...
Add spells to the RPG API's reference data in `rpg-backend/data/spells.json`:
...
### Quests, Scenarios, and Story Management

The game uses an **LLM-driven quest system** where the agent decides when to progress quests based on player actions.

#### Quest System (LLM-Driven)

Quests are stored in the database but progression is controlled by the narrator agent:

- **Quest Storage**: Quests live in the RPG API (`/quests/` endpoints) with objectives, progress tracking, and rewards
- **LLM Progression**: The narrator agent has function tools to manage quests:
  - `get_available_quests()` - List quests the player can accept
  - `get_active_quests()` - Get current quests with objective IDs
  - `accept_quest(quest_id)` - Accept a new quest
  - `progress_quest_objective(quest_id, objective_id)` - Update objective progress
  - `complete_quest(quest_id)` - Finish a quest and claim rewards
- **Agent Responsibility**: After every player action (talking, traveling, combat), the agent checks if the action matches any quest objectives

#### Scenarios and Zones

- **Scenarios** (`/scenario/` endpoints): Narrative events triggered by location, items, health, etc. with multiple weighted outcomes
- **Zones** (`/location/zones/` endpoints): Named locations with exits for immersive navigation ("tavern door" vs "north")

Initial game content is seeded via `rpg-backend/src/seed.py`. From the `rpg-backend/` directory, run `python -m src.seed --reseed` to reset the database with starter quests, scenarios, and zones.
